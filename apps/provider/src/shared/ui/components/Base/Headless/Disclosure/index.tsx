...
Disclosure.Button = ({
  ...
  const group = useContext(groupContext);
  const { close, key: disclosureKey } = disclosure;
  const { selectedIndex } = group;

  useEffect(() => {
    if (selectedIndex !== null && selectedIndex !== disclosureKey) {
      close();
    }
  }, [close, disclosureKey, selectedIndex]);

  return (
    <HeadlessDisclosure.Button
      as="button"
      className={twMerge([
        "outline-none py-4 -my-4 font-medium w-full text-start dark:text-slate-400",
