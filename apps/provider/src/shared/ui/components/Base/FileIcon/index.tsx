...
function FileIcon({ variant, type, src, className, ...props }: FileIconProps) {
  ...
  return (
    <div className={className} {...props}>
      ...
      >
        {variant == "file" && (
          <div className="absolute top-0 bottom-0 start-0 end-0 flex items-center justify-center m-auto font-medium text-slate-500/60">
            {type}
          </div>
        )}
        {variant == "image" && (
          <div className="absolute top-0 start-0 w-full h-full image-fit">
            <img
