...
export const useStore = create<AuthStore>((set, get) => ({
  ...
  lookupPhone: async (phone) => {
    ...
    try {
      const response = await postJson(
        "/api/provider/auth/phone-lookup",
        {
          phone,
        },
      );
  ...
  registerProvider: async ({
    ...
    if (isDemoMode) {
      const expectedToken = `demo-registration-${phone}`;
  ...
  resetPassword: async ({
    ...
    if (isDemoMode) {
      const expectedToken = `demo-password-reset-${phone}`;
  ...
  logout: async () => {
    ...
    try {
      ...
      // Local auth state is cleared even when the server is unavailable.
    }
  },
}));
